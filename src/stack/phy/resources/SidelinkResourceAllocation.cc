...
void SidelinkResourceAllocation::handleUpperMessage(Packet* pkt)
{
	EV<<"SidelinkResourceAllocation::handleUpperMessage: "<<pkt->getName()<<endl;

	auto lteInfo = pkt->getTagForUpdate<UserControlInfo>();

	if (pkt->isName("LteMode4Grant") )
	{
		nodeType_ = UE;}
	else if ( pkt->isName("LteMode3Grant"))
	{
		nodeType_ = ENODEB;
	}
	...
	if (lteInfo->getFrameType() == SIDELINKGRANT)
	{
		...
		auto grant = pkt->peekData<LteSidelinkGrant>();
		...
		if (nodeType_ ==UE)
		{
			...
			if (getAllocatedBlocksPrevious() == 0 || getReselectionCounter()==0)
			{
				...
				EV<<"Compute CSR"<<grant->getPeriodic()<<endl;
				computeCSRs(pkt,nodeType_);
				lteInfo->setUserTxParams(grant->getUserTxParams());
				lteInfo->setGrantedBlocks(grant->getGrantedBlocks());
...
LteAirFrame* SidelinkResourceAllocation::createSCIMessage(cMessage* msg, Packet* pkt)
{
	EV << NOW << " SidelinkResourceAllocation::createSCIMessage - Start creating SCI..." << endl;
	auto grant = pkt->peekAtFront<LteSidelinkGrant>();
...
void SidelinkResourceAllocation::computeCSRs(Packet* pkt, LteNodeType nodeType_) {
	auto grant = pkt->peekData<LteSidelinkGrant>();
	...
	if ((nodeType_==ENODEB) || (getAllocatedBlocksPrevious() == 0 && getReselectionCounter()==0 && nodeType_ == UE))
	{
		...
		sciGrant_ = grant->dup();
		sciGrant_->setStartingSubchannel(0);
	}
	...
	EV<<"Total number of Resource Blocks for SCI+Data: "<<totalPRBsPerSubframe<<endl;
	...
	double maxLatency = sciGrant_->getMaximumLatency();
	...
	EV<<"Number of candidate subframes within selection window: "<< candidateSubframes.size()<<endl;
	int candidateSubframeInitial = 0;
	candidateSubframeInitial =candidateSubframes.size();


	unsigned int Prsvp_TX = sciGrant_->getPeriod(); //Resource reservation interval
	unsigned int  Prsvp_TX_prime = (Prsvp_TX*pStep_)/100;
	std::vector<double> possibleRRIs = sciGrant_->getPossibleRRIs();
	std::vector<double>::iterator k;

	if ((getAllocatedBlocksPrevious() == 0 || getReselectionCounter()==0) ||nodeType_ == ENODEB)
	{
		//Allocating subchannels
		int cResel =sciGrant_->getResourceReselectionCounter();
		setReselectionCounter(cResel);
		...
		sciGrant_->setTotalGrantedBlocks(RBIndicesSCI.size()+RBIndicesData.size());
		setPreviousSubchannelsData(RBIndicesData);

		EV<<"First transmission: "<<startFirstTransmission<<endl;
		sciGrant_->setStartTime(startFirstTransmission);
		sciGrant_->setFirstTransmission(true);
		...
		if(eraseSubframe.size()>(0.8*candidateSubframeInitial))
		{
			...
			for (int k=0; k<eraseSubframe.size();k++)
			{
				optimalCSRs=selectBestRSSIs(eraseSubframe, sciGrant_,eraseSubframe[k]);
			}
		}
		else
		{
	...
	if (getAllocatedBlocksPrevious() > 0 && getReselectionCounter()>0 && nodeType_==UE) //only for mode 4
	{
		...
		sciGrant_->setStartTime( FirstTransmission);
	...
	candidateResourcesMessage->setSidelinkGrant(sciGrant_);
	LtePhyBase* phy=check_and_cast<LtePhyBase*>(getParentModule()->getSubmodule("phy"));
...
simtime_t SidelinkResourceAllocation::sidelinkSynchronization()
{
	...
	EV<<"Transmitting Synchronization subframes ..."<<endl;
