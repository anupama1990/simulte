...
void  UmTxEntity::rlcPduMake(int pduLength, LteControlInfo* info)
{
	...
	pkt->addTagIfAbsent<FlowControlInfo>()->setSourceId(info->getSourceId());
	pkt->addTagIfAbsent<FlowControlInfo>()->setDestId(info->getDestId());
	pkt->addTagIfAbsent<FlowControlInfo>()->setLcid(info->getLcid());
	pkt->addTagIfAbsent<FlowControlInfo>()->setDirection(info->getDirection());
	//pkt->addTagIfAbsent<FlowControlInfo>()->setFlowControlInfo(info);
	// send to MAC layer
