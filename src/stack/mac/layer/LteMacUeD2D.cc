	...
	if(!bsrAlreadyMade)
	{
		...
		for (it = scheduleList_->begin(); it != scheduleList_->end(); it++)
		{
			...
			if (pit == macPduList_.end())
			{
				...
				macPkt->addTagIfAbsent<UserControlInfo>()->setUserTxParams(sidelinkgrant->getUserTxParams());
...
void LteMacUeD2D::handleMessage(cMessage* msg)
{
	...
	if (strcmp(pkt->getName(), "CSRs")== 0)
	{
		...
		SidelinkConfiguration* slConfig = check_and_cast<SidelinkConfiguration*>(getParentModule()->getSubmodule("mode4config"));
		slConfig->macHandleSps(candidatesPacket->getCSRs(), rrcCurrentState, candidatesPacket->getSidelinkGrant());
		mode4Grant=slConfig->getSidelinkGrant();
		expirationCounter_ = mode4Grant->getExpirationCounter();
		periodCounter_= mode4Grant->getPeriodCounter();
		...
		send(csrsprevious,down_[OUT_GATE]);
		return;
	}
	...
	if (incoming == down_[IN_GATE])
	{
		...
		if (userInfo->getFrameType()==RACPKT)
		{
			...
			auto mode3GrantPkt = new Packet("Mode3GrantRequest"); //packet creation
			auto mode3Grant = makeShared<LteSidelinkGrant>();  //Sidelink grant header
			UserControlInfo* uinfo = new UserControlInfo();
			uinfo->setSourceId(getMacNodeId());
			uinfo->setDestId(getMacCellId());
			mode3GrantPkt->insertAtFront(mode3Grant);
			uinfo->setFrameType(MODE3GRANTREQUEST);

			sendLowerPackets(mode3GrantPkt);
			return;
		}
...
void LteMacUeD2D::handleSelfMessage()
{
	...
	EV << NOW << " LteMacUeD2D::hna " << nodeId_ << " - HARQ process " << (unsigned int)currentHarq_ << endl;
