...
void LteMacUe::initialize(int stage)
{
	...
	else if (stage == INITSTAGE_NETWORK_LAYER)
	{
		...
		EV<<"LteMacUe:8 node id: "<<info->id<<endl;
		...
		NetworkInterface * interfaceEntry = interfaceTable->findInterfaceByName("cellular");
...
{
	EV<<"LteMacUe::bufferizePacket(): "<<endl;
...
bool LteMacUe::bufferizePacket(cPacket* pktAux)
{
	...
	if (checkIfHeaderType<LteRlcPduNewData>(pkt))
	{
		...
		if (it == macBuffers_.end())
		{
			...
			EV << "LteMacBuffers : Using new buffer on node 1: " <<
					MacCidToNodeId(cid) << " for Lcid: " << MacCidToLcid(cid) << ", Bytes in the Queue: " <<
					vqueue->getQueueOccupancy() << "\n";
		}
		else
		{
			...
			EV << "LteMacBuffers : Using old buffer on node 1: " <<
					MacCidToNodeId(cid) << " for Lcid: " << MacCidToLcid(cid) << ", Space left in the Queue: " <<
					vqueue->getQueueOccupancy() << "\n";
		}
	...
	if (it == mbuf_.end())
	{
		...
		EV << "LteMacBuffers : Using new buffer on node 2: " <<
				MacCidToNodeId(cid) << " for Lcid: " << MacCidToLcid(cid) << ", Space left in the Queue: " <<
				queue->getQueueSize() - queue->getByteLength() << "\n";
	}
	else
	{
		...
		EV << "LteMacBuffers : Using old buffer on node 21: " <<
				MacCidToNodeId(cid) << " for Lcid: " << MacCidToLcid(cid) << "(cid: " << cid << "), Space left in the Queue: " <<
				queue->getQueueSize() - queue->getByteLength() << "\n";
	}
...
void LteMacUe::handleUpperMessage(cPacket* pktAux)
{
	...
	EV<<"LteMacUe::handleUpperMessage"<<endl;
	auto pkt = check_and_cast<Packet *>(pktAux);
	...
	MacCid cid = ctrlInfoToMacCid(lteInfo->dup());
	EV<<"cid: "<<cid<<endl;
	EV<<"Request sdu: "<<requestedSdus_<<endl;
	// bufferize packet
	bufferizePacket(pkt);
	...
	if (strcmp(pktAux->getName(), "lteRlcFragment") == 0)
	    {
	if (requestedSdus_ == 0)
	{
